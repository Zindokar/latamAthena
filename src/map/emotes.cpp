...
{
	std::tm tmStruct = {};
	tmStruct.tm_year = static_cast<int32>(dateValue / 10000) - 1900;
	tmStruct.tm_mon = static_cast<int32>((dateValue / 100) % 100) - 1;
	tmStruct.tm_mday = static_cast<int32>(dateValue % 100);
...
void emotes_use(map_session_data* sd, int16 packId, int16 emoteId)
{
	...
	if (battle_config.client_reshuffle_dice && emoteId >= ET_DICE1 && emoteId <= ET_DICE6)
		emoteId = static_cast<emotion_type>(rnd() % 6 + ET_DICE1);
...
void emotes_expantion_buy(map_session_data* sd, int16 packId, int16 itemId, int8 amount)
{
	...
	if (amount > 0) {
		...
		int32 inv_index = pc_search_inventory(sd, itemId);
