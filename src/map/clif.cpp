...
void clif_display_pinfo( map_session_data& sd ){
	...
	// Client is printing this msgstringtable: MSI_BASIC_EXP_MSG2_EXCEPT_PREMIUM
	// It's like: %.1f%% (Internet Cafe %.1f%% + TPLUS %.1f%% + %s Server %.1f%%)
	// Should be like: %.1f%% (Basic %.1f%% + VIP %.1f%% + %s Server %.1f%%)
	// 0 = PCRoom (should be basic always 100%)
	// 1 = TPLUS (should be premium values from conf_battle, for example 30%)
	// 2 = Server rates (should be the extra rates, for example for a 2x server, 100%)
	// All should be = 230%
	// 
	// EXP: 230% (Basic 100% + VIP 30% + Freya Server 100%)
	// DROP: 170% (Basic 100% + VIP 30% + Freya Server 50%)
	// DEATH: 0% (Basic 300% + VIP 100% + Freya Server 200%) 
	//
	// 0: PCRoom = Basic
	// 3: PCafe = Premium
	// 2: PServer = Server
	...
	* EXP 
	*/
	//0:PCRoom - always 100% or under
	details_bexp[PINFO_BASIC] = battle_config.base_exp_rate;
	if (details_bexp[PINFO_BASIC] >= 100) {
		details_bexp[PINFO_BASIC] = 100;
	}
	else
		details_bexp[PINFO_BASIC] = 100 - details_bexp[PINFO_BASIC]; // for example 80% 
	...
	//2: Server - if base under 100% is 0%, if over 100% = custom_rate - 100 (custom rate 200% then is 100%)
	...
	if (pc_isvip(&sd)) {
		details_bexp[PINFO_CAFE] = battle_config.vip_base_exp_increase * battle_config.base_exp_rate / 100; // We just get the conf value
	}
	...
	/**
		...
		*/
	//0: PCRoom - Always 100% or under
	details_drop[PINFO_BASIC] = battle_config.item_rate_common;
	if (details_drop[PINFO_BASIC] >= 100) {
		details_drop[PINFO_BASIC] = 100;
	}
	else
		details_drop[PINFO_BASIC] = 100 - details_drop[PINFO_BASIC]; // for example 80% 
	...
	//2: Server rates - if base under 100% is 0%, if over 100% = custom_rate - 100 (custom rate 200% then is 100%)
	...
	if (pc_isvip(&sd)) {
		details_drop[PINFO_CAFE] = battle_config.vip_drop_increase * battle_config.item_rate_common / 100; // We just get the conf value
	}
	...
	*/
	...
	//! Fix suggestion: Use the final calculation with the substraction
	//! DEATH: 100% (Basic 300% + VIP 200% + Freya Server 0%)
	//! Another suggestion:
	//! For negative death penalty values we can change from MSI_BASIC_EXP_MSG2_EXCEPT_PREMIUM to MSI_TAIWAN_PERSONALINFO_EXPMSG
	//! MSI_TAIWAN_PERSONALINFO_EXPMSG,EXP: %.1f%% (Basic %.1f %% + Premium %.1f%% + % server %.1f%%)
	//! MSI_TAIWAN_PERSONALINFO_DROPMSG,DROP: %.1f%% (Basic %.1f%% + Premium %.1f%% + % server %.1f%%)
	//! MSI_TAIWAN_PERSONALINFO_DEATHMSG,DEATH: %.1f%% (Basic %.1f%% - Premium %.1f%% - % server %.1f%%)
	//0:PCRoom - base kRO penalty
	details_penalty[PINFO_BASIC] = 300;
	...
	//2:Server - We get the substraction to the kro Base or addition
	details_penalty[PINFO_SERVER] = battle_config.death_penalty_base;
	if (details_penalty[PINFO_SERVER] >= 300)
		details_penalty[PINFO_SERVER] = details_penalty[PINFO_SERVER] - 300;
	else
		details_penalty[PINFO_SERVER] = 300 - details_penalty[PINFO_SERVER];

	//3:TPLUS - Premium
	details_penalty[PINFO_CAFE] = 0;
	if (pc_isvip(&sd)) {
		details_penalty[PINFO_CAFE] = battle_config.vip_exp_penalty_base - (details_penalty[PINFO_BASIC] + details_penalty[PINFO_SERVER]);
	}
	...
	if (pc_isvip(&sd))
		p->total_death = battle_config.vip_exp_penalty_base * 1000; // always vip base rate for VIP
	else
		p->total_death = 1000 * (details_penalty[PINFO_BASIC] + details_penalty[PINFO_SERVER]);
...
void clif_parse_emotion2(int32 fd, map_session_data* sd)
{
#if PACKETVER >= 20230802
	nullpo_retv(sd);
...
void clif_parse_emotion_expansion_request(int32 fd, map_session_data* sd)
{
#if PACKETVER >= 20230802
	nullpo_retv(sd);
